...
def load_system_json_dump(dump_file: Path) -> List[SystemSpansh]:
    timer = Timer("__load_data_dump()")

    system_dto_list: List[SystemSpansh] = []
    with dump_file.open("r") as f:
        systems_dict = json.load(f)
        system_dto_list = SystemSpansh.schema().loads(systems_dict, many=True)
...
def load_spansh_populated_systems_dump() -> Dict[str, System]:
    ...
    system_dto_list: List[SystemSpansh] = load_system_json_dump(dump_file)
